...
class SgmntDisplay extends HTMLElement {
    ...
    constructor() {
      ...
      makeChar(char, segments) {
        ...
        const cssRules = segmentClasses.map(
            (segment) => ` #ltr-${char} .${segment} 
            {color: ${this.background};}`).join('\n');
        return cssRules;
      }
      ...
      `
        ...
        .sgmnt  {
            color: ${this.sgmntColor};
        }
        .sgmnt-display {
            --size: ${this.sgmntSize}px;
    ...
    if(this.text[4] == 'd') day = '  ' + now.toLocaleString("en-US",{weekday: this.dayPref}).toUpperCase(); 
    if(this.text[2] == 'm') minutes = ':' + String(now.getMinutes()).padStart(2, '0');
    if((this.text[3] == 's') && (this.text[2] == 'm')) seconds = ':' + String(now.getSeconds()).padStart(2, '0');
    ...
    if(!this.timeStarted) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if(((isNaN(text[i]) && text[i] !== ':' && text[i] !== '.') || (text[i] === ' ') && (!isNaN(text[i-1])))) {
                console.log('-----', [text[i]])
            ...
            if(this.sgmtSmall && (!isNaN(text[i]) || text[i] === '.')) {
                ...
                } else {dateContainer.appendChild(ltr);}
